...
TEST(SerializerTest, SingleValue) {
	...
	u8 expected[] = { 0x04, 0xBF, 0xFF, 0xFF, 0xFE };
	ASSERT_EQ(make_vector(expected), s.data());
}
...
TEST(SerializerTest, Clear) {
	...
	s << AmfDouble(-1.2);
	u8 expected[] = { 0x05, 0xBF, 0xF3, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33 };
	ASSERT_EQ(make_vector(expected), s.data());
}
...
TEST(SerializerTest, MultipleDoubleValues) {
	...
	ASSERT_EQ(make_vector(expected2), s.data());
}
